...
> 注意：`等待 N 秒`、`滑行` 走的是真实时间，不会跟着慢动作变慢——这是 Scratch 的计时方式决定的。
> 另外 Scratch 自带的「Debug」菜单不是调试器，是一组**排错技巧提示**（拆小问题、检查数值、读出来…），对小孩也挺有用。
...
- **扩展 worker**：scratch-vm 加载自定义扩展时写死了 `new Worker('./extension-worker.js')`，相对页面解析成
  `/extension-worker.js`，所以根路径也挂了一份（否则将来加自定义积木扩展会 404）
- **调试面板怎么接的**：scratch-vm 的步进就是 `runtime` 上的一个 `setInterval(_step, 33ms)`，句柄在
  `runtime._steppingInterval`。暂停 = 清掉它，慢放 = 用更大的间隔重建，单步 = 手动调一次 `_step()`。
